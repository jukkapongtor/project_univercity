...
<?php
			...
			echo "<table class='table table-hover table-striped' style='font-size:13px'>";      
				$total_price=0;
				$query_orderdetail = mysqli_query($_SESSION['connect_db'],"SELECT product.product_id,product.product_name,size.size_name,order_detail.price,order_detail.amount,type.type_name,type.type_name_eng FROM order_detail LEFT JOIN product ON order_detail.product_id = product.product_id LEFT JOIN size ON order_detail.size_id = size.product_size LEFT JOIN type ON product.product_type = type.product_type WHERE order_detail.order_id = '$_POST[order_id]'")or die("ERROR : order function line 111");

...
<?php
	$img =$_FILES['employee_img']['name'];
		...
		$day = substr($_POST['birth_date'],0,2);
        $month = substr($_POST['birth_date'],3,2);
        $year = substr($_POST['birth_date'],6,4);
        $_POST['birth_date'] = "$year-$month-$day";
...
?>
<script>swal({title:'',text: 'แก้ไขข้อมูลพนักงานเรียบร้อยแล้ว',type:'success',showCancelButton: false,confirmButtonColor: '#1ca332',confirmButtonText: 'ยันยัน',closeOnConfirm: false },function(){ window.location='../#ajax/employee_manage.php'})</script>
</body>
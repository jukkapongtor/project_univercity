...
<?php
	...
	if(empty($_POST['product_name'])||empty($_POST['product_type'])||empty($_POST['product_quality'])||empty($_POST['product_detail'])||empty($_POST['product_size'])||empty($_FILES['product_image'])){
		echo "<script>swal({title:'',text: 'กรุณากรอกข้อมูลให้ครบถ้วน',type:'error',showCancelButton: false,confirmButtonColor: '#f27474',confirmButtonText: 'ยันยัน',closeOnConfirm: false },function(){ window.location='../#ajax/product_add.php';})</script>";
	}else{
		$query_product = mysqli_query($_SESSION['connect_db'],"SELECT product_name FROM product WHERE product_name='$_POST[product_name]'")or die("ERROR : product insert line 22");
		...
		if(empty($row)){
			...
			*/
			...
			echo "<script>swal({title:'',text: 'ชื่อสินค้านี้มีในระบบแล้ว กรุณาเพิ่มชื่อสินค้าอื่น',type:'error',showCancelButton: false,confirmButtonColor: '#f27474',confirmButtonText: 'ยันยัน',closeOnConfirm: false },function(){ window.location='../#ajax/product_add.php';})</script>";
		}
	}
